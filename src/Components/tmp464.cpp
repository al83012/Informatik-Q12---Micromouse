...
float TMP464::readLocalTemperature() {
    uint16_t rawTemp = Esp32::i2c_readRegister16(tmp464_componentVars.I2C_ADDRESS, tmp464_componentVars.REG_LOCAL_TEMP);
...
float TMP464::readRemoteTemperature(uint8_t channel) {
    ...
    uint16_t rawTemp = Esp32::i2c_readRegister16(tmp464_componentVars.I2C_ADDRESS, tmp464_componentVars.REG_REMOTE_TEMP + (channel - 1)); 
    return convertToCelsius(rawTemp);
}
